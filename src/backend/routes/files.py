...
def list_files():
    """List minute folders in the capture directory."""
    try:
        if request.args.get('subdir'):
        ...
        return jsonify({
            'status': 'success',
            'data': files,
            'current_dir': request.args.get('subdir', '')
        })
...
def list_capture_minutes():
    """Alias for listing capture minutes."""
    return jsonify({
        'status': 'success',
