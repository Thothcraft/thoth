...
def test_get_current_data():
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/data/current")
        print(f"Status Code: {response.status_code}")
        print(f"Response: {json.dumps(response.json(), indent=2)}")
        return response.status_code == 200
    except Exception as e:
...
def test_get_data_history():
    ...
    try:
        ...
        response = requests.get(f"{BASE_URL}/api/data/history", params=params)
        print(f"Status Code: {response.status_code}")
        data = response.json()
        print(f"Found {len(data.get('data', []))} records")
        print(f"Sample record: {json.dumps(data.get('data', [{}])[0], indent=2) if data.get('data') else 'No data'}")
        return response.status_code == 200
    except Exception as e:
        print(f"Error: {e}")
        return False
...
def test_collection_control():
    ...
    print("\nStarting collection...")
    try:
        response = requests.post(f"{BASE_URL}/api/collection/start")
    ...
    print("\nStopping collection...")
    try:
        response = requests.post(f"{BASE_URL}/api/collection/stop")
